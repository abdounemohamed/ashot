...
//        return $historical_rates;
    ...
    public function getBankRates($banks, $currencies)
    {
        ...
        /*** WORK +  CHANGE LINK - ***/
        $anelik = $this->getAnelikRates('https://www.idbank.am/en/', $currencies);
...
//        /*** WORK + CHANGE LINK - ***/
        ...
        /*** WORK +  CHANGE LINK - ***/
        $ardshin = $this->getArdshinRates('https://www.ardshinbank.am/', $currencies);
        ...
        /*** WORK + CHANGE LINK - ***/
        //$evoka = $this->getEvokaRates('https://www.evocabank.am/hy/', $currencies);
...
//        /*** WORK + ***/
            ...
            foreach($searchResultsBlocks as $searchResults) {
                ...
                foreach ($searchResults->find('ul') as $searchResultsItems) {
                    ...
                    foreach ($searchResultsItems->find('li') as $element) {
                        ...
                        $elementRework = str_replace([', ', '</sup>'], '-', $currentElement);
                        $elementRework = str_replace(' ', '-', $elementRework);
                        $currentRates = explode("-", $elementRework);

                        if (empty($currentRates) || $currentRates[0] == 'Buy') continue;
                        ...
                        if ($currentCurrencyId !== null) {
                            if (!empty($currentRates[0])) {
                                $data[$currentCurrencyId][$buy] = (float)floatval(str_replace(",", ".", $currentRates[3]));
                                $data[$currentCurrencyId][$sell] = (float)floatval(str_replace(",", ".", $currentRates[5]));
                                $cc++;
                            }
                        }
    ...
    private function getArdshinRates($bankRateLink, $currencies)
    {
        ...
        try {
            $bankRateLink = 'https://www.ardshinbank.am/';
            $data = array();
            $content = $this->bankCurlInit($bankRateLink);
            $html = HtmlDomParser::str_get_html($content);
            $searchResultsTables = $html->find("tabel.tg");
            $index = 0;
            $buy = "buy";
            $sell = "sell";
    ...
    private function getArmEconomRates($bankRateLink, $currencies)
    {
        try {
            $data = array();
            $bankLinkApiRates = 'https://arusapi.aeb.am:8093/api/Rates';
            $content = $this->bankCurlInit($bankLinkApiRates);

            $searchResultsBloks = json_decode($content, true);
            $index = 0;
            $buy = "buy";
            $sell = "sell";

            foreach($searchResultsBloks['rates'] as $searchResults) {

                $currentCurrencyId = null;

                $col1 = trim($searchResults['currency']);
                $col2 = trim($searchResults[$buy]);
                $col3 = trim($searchResults[$sell]);
                $col4 = trim($searchResults['buyNC']);
                $col5 = trim($searchResults['sellNC']);
                ...
                if ($currentCurrencyId !== null) {
                    $data[$currentCurrencyId][$buy] = (float)floatval(str_replace(",", ".", $col2));
                    $data[$currentCurrencyId][$sell] = (float)floatval(str_replace(",", ".", $col3));
                    $data[$currentCurrencyId]['buyNonCash'] = (float)floatval(str_replace(",", ".", $col4));
                    $data[$currentCurrencyId]['sellNonCash'] = (float)floatval(str_replace(",", ".", $col5));
                }
            }
    ...
    private function getArtsakhRates($bankRateLink, $currencies)
    {
        ...
        try {
            $bankRateLink = 'https://www.artsakhbank.am/';
            $data = array();
            $content = $this->bankCurlInit($bankRateLink);

            $html = HtmlDomParser::str_get_html($content);
            $selectorName = "div.tabs_container div.exchange_1 ul.exchange_list li ul";
            $buy = "buy";
            $sell = "sell";
            ...
            for($i = 0 ; $i < 2; $i ++){
                ...
                foreach ($html->find($selectorName) as $element) {
                    ...
                    $col1 = trim(@$element->find('li')[0]->plaintext);
                    $col2 = trim(@$element->find('li')[1]->plaintext);
                    $col3 = trim(@$element->find('li')[2]->plaintext);
                    if ($col1 == "RUR") {
                        $col1 = "RUB";
                    }
                ...
                $buy = "buyNonCash";
                $sell = "sellNonCash";
                $selectorName = "div.tabs_container div.exchange_2 ul.exchange_list li ul";
            }
    ...
    private function getVtbRates($bankRateLink, $currencies)
    {
        try {
            $data = array();
            $bankRateLink = 'https://www.vtb.am/';

            $content = $this->bankCurlInit($bankRateLink);

            $html = HtmlDomParser::str_get_html($content);
            $searchResults = $html->find('ul.current-money li');
            ...
            for ($i = 0 ; $i < 2 ;$i ++) {
                ...
                foreach ($searchResults as $element) {
                    ...
                    $col1 = trim(@$element->find('span')[0]->innertext);
                    $col1 = str_replace("/AMD", "", strip_tags($col1));

                    $col2 = trim(strip_tags(@$element->find('span')[$fstIndex]->innertext));
                    $col3 = trim(strip_tags(@$element->find('span')[$secIndex]->innertext));
